...
const categories = computed(() => {
  // 将注释的代码放入集合中增加全部Tab
  // strings.get("all_categories"),
  return [...new Set(items.value.map((item) => item.props.category))].filter(
    (category) => category
  )
})
...
:root {
  --project-logo-size: min(
    clamp(140px, 20vh, 170px),
    clamp(140px, 10.5vw, 170px)
  );
  @include media-breakpoint-down(lg) {
    --project-logo-size: min(
      clamp(80px, 20vh, 110px),
      clamp(70px, 20.5vw, 110px)
    );
  }
}
